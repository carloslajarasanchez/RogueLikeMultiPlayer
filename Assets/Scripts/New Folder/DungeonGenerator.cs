...
public class DungeonGenerator : MonoBehaviour
{
    ...
    {
        RoomTemplates templates = Object.FindFirstObjectByType<RoomTemplates>();
        ...
        // — Spawn del Player en la sala de inicio —
        if (_playerPrefab != null)
        {
            Vector3 spawnPos = new Vector3(0f, 1f, 0f);
            GameObject player = Instantiate(_playerPrefab, spawnPos, Quaternion.identity);
            Main.CustomEvents.OnPlayerSpawned?.Invoke(player.transform); // pasamos el transform directamente
        }
    }
